"""Guard against post-split ReferenceError regressions on inline event handlers.
...
Inline event handlers (``onclick=``, ``onmousedown=``, ``oninput=``,
``onsubmit=``, …) emitted from ``render.py``, Jinja templates, or
``dashboard.html`` are resolved against ``window`` at event time, so every
such handler must be re-exported by the explicit
...
INLINE_HANDLER_RE = re.compile(
    r"on(?:click|mousedown|mouseup|mouseover|mouseout|mouseenter|mouseleave|mousemove"
    r"|input|change|submit|focus|blur|keydown|keyup|keypress"
    r"|dragstart|dragend|dragover|drop|wheel|contextmenu"
    r')=\\?"([A-Za-z_][A-Za-z0-9_]*)\('
)
# JS reserved words that can legitimately appear as the first token of an
# inline handler body (e.g. `onclick="if(event.target===this)close()"`).
...
def _collect_onclick_handlers() -> dict[str, list[Path]]:
    ...
    for path in sources:
        ...
        for name in INLINE_HANDLER_RE.findall(text):
...
def test_every_inline_onclick_handler_is_exported_on_window():
    ...
    assert handlers, (
        'Expected at least one inline `on<event>="<name>(` in render.py / templates / '
    ...
    if missing:
        lines = [
            "Inline event handlers missing from `Object.assign(window, { ... })` in dashboard-main.js:"
        ]

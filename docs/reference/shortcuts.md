...
These live at the dashboard level and can fire from outside the terminal pane (e.g. toggle it open from anywhere). Configurable via the `[terminal]` block in `config.toml` / `preferences.yml`. Shortcut strings follow the `KeyboardEvent.key` convention — modifiers are `Ctrl`, `Shift`, `Alt`, `Meta`.
...
`[terminal]` shortcut changes saved via the gear modal take effect **live** — the page re-reads `/config` on save and rebuilds the parsed shortcut specs. No restart needed. Changes made by hand-editing `config.toml` / `preferences.yml` require a page refresh.
